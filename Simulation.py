...
if __name__ == "__main__":
    ...
    # # ----------------------------------------------------------------------------
    df = pd.read_csv("data_edge", index_col=0)
    hm = df.to_numpy(float)
    # exp=np.sum(hm,axis=1,dtype= float)
    # exp_budget=1-(exp/exp[0])
    # print(exp_budget)

    # b 30:  462.0, b 50:  467.0 ,b 70:  468.0
    ...
    # plt.title(f"Reduction of infection, with varying interdiction budgets")
    # plt.grid(True, alpha=0.3)
    # plt.show()

    plt.plot(exp_budget, linewidth=2, label="edge")
    plt.plot(exp_budget_2, linewidth=2, label="semi edge")
    plt.plot(exp_budget_3, linewidth=2, label="edge mzn")
    plt.xlabel("Budget")
    ...
    # num_infected = np.array(hm[:,-1])
    # print(num_infected)
    # b_50=0
    # b_70=0
    # b_90=0
    # baseline=num_infected[0]
    # red=1-num_infected/baseline

    # for b, r in enumerate(red):
    #     if b_50==0 and r>=0.5:
    #         b_50=b
    #     elif b_70==0 and r>=0.7:
    #         b_70=b
    #     elif b_90==0 and r>=0.9:
    #         b_90=b
