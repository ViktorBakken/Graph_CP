...
import numpy as np
def cascade(t=2, n=100,spread=0.2,budget=2,graph_edges=None, init_infected=None, mode="SI",budget_type="edge",intervention_step=None,displ=False,T=None, layout=None):
    # Initialization
    ...
    if init_infected is None:
        infected = {88,18,77,46,68,0,8,52,66,11}
    else:
                ...
                match budget_type:
                    case "node":
                        ...
                        for node in rem_nodes:
                            removed.add(node)
                    case "edge mzn":
                        edges,_=interdiction_minizinc(solver_name="gurobi",num_nodes=n,budget=budget,infected_nodes=infected,critical_nodes=T, graph_edges=edges,interdiction_type="edge", displ=displ)                      
